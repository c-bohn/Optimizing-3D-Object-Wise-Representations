...
INSIDE_BLENDER = True
try:
    import bpy, bpy_extras, bmesh, _cycles
...
parser.add_argument('--properties_json', default='data/properties_3drec.json',
                    help="JSON file in DeepSDF format.")
parser.add_argument('--shape_dir', default='data/shapes',
                    help="Directory where .blend files for object models are stored")
parser.add_argument('--material_dir', default='data/materials',
                    help="Directory where .blend files for materials are stored")
parser.add_argument('--shape_color_combos_json', default=None,
                    help="Root directory of ShapeNetCore.v2")
parser.add_argument('--mode',
                    help="Select one mode from ['rnd', 'basic', 'single-rot']. ")
...
parser.add_argument('--obj_size', default='small',
                    help="Select type of size fromm ['small' (default), 'large']")
...
parser.add_argument('--save_objfiles', type=int, default=0,
                    help="Setting --save_objfiles 1 will cause the object file for " +
                         "each rendered object to be stored.")
...
parser.add_argument('--color_bg', default=0, type=int,
                    help="Setting --color_bg 1 uses bg color defines in property file. ")
...
def main(args):
    ...
    obj_template = '%s%%0%dd_obj.ply' % (prefix, num_digits)
    ...
    if not os.path.isdir(os.path.join(args.output_dir, 'depth')):
    ...
    if args.save_objfiles == 1 and not os.path.isdir(os.path.join(args.output_dir, 'objects')):
        os.makedirs(os.path.join(args.output_dir, 'objects'))
    if args.save_objfiles == 1 and not os.path.isdir(os.path.join(args.output_dir, 'objects/original')):
        os.makedirs(os.path.join(args.output_dir, 'objects/original'))

    if args.mode not in ['rnd', 'basic', 'single-rot']:
        print('[ERROR] Mode ', args.mode, ' not in [rnd, basic, single-rot].')
        exit(0)
...
def render_scene(args,
    ...
    # Fix issue with speckles in depth map. Remove transparent shader. TODO: Check if this can be done better
    for mat in bpy.data.materials:
        n = mat.node_tree.nodes.get("Transparent BSDF", None)
        if n is not None:
    ...
    if args.color_bg == 1:
        ...
        with open(args.properties_json, 'r') as f:
            ...
            rgb = [c/255. for c in properties["color_bg"]]
    ...
    while True:
        # try:
            # Save obj (mesh file)
            if args.save_objfiles:
                for i, obj in enumerate(blender_objects):
                    bpy.context.scene.objects.active = bpy.data.objects[obj.name]

                    name = (obj.name).split('/')[1]
                    objfile = output_objfile.replace('_obj', '_' + name)
                    print('-- Save object to', objfile)
                    bpy.ops.export_mesh.ply(filepath=objfile)

            # Multiple images with moved objects
            for n in range(args.num_views):

                objects = move_objects(scene_struct, objects, blender_objects, new_pos=(n==0))  # new_pos=(n==0) if objects should just be rotated
    ...
    scene_struct['relationships'] = compute_all_relationships(scene_struct, n_views=args.num_views)
...
def add_random_objects(scene_struct, num_objects, args, output_index):
    ...
    print(args.properties_json)
    with open(args.properties_json, 'r') as f:
        print(f)
        properties = json.load(f)
        object_list = []
        object_size = {}
        for (c, a) in properties['ShapeNetV2'].items():
            object_list += [os.path.join(c, o) for o in a['shapes']]
            if not c in object_size.keys():
                object_size[c] = a['sizes']
    ...
    for i in range(num_objects):
        ...
        if args.mode == 'rnd':
            obj_name = random.choice(object_list)  # TODO: setting object
            obj_cat = obj_name.split('/')[0]
            size = object_size[obj_cat][args.obj_size]  # TODO: setting size
        elif args.mode == 'basic':
            obj_name = object_list[output_index]
            obj_cat = obj_name.split('/')[0]
            size = object_size[obj_cat][args.obj_size]
        elif 'single-rot':
            obj_name = random.choice(object_list)
            obj_cat = obj_name.split('/')[0]
            size = object_size[obj_cat][args.obj_size]

        color_inst = tuple([random.random() for _ in range(3)])

        # Scale transformation factors along different axes
        scale_vars = [1., 1., 1.]

        # Actually add the object to the scene
        utils.add_object_shapenet(args.shape_dir, obj_name, size, (0., 0.), theta=0., scale_vars=scale_vars)
        obj = bpy.context.object
        # size = np.linalg.norm(obj.dimensions)/2.
        size = 0.5 * max(obj.dimensions)  # scaling factor w.r.t. normalized box [-1, 1], padding is not taken into account here
...
def move_objects(scene_struct, objects, blender_objects, new_pos=0):
    """
    ...
    for i, obj in enumerate(blender_objects):
        ...
        if args.mode == 'rnd' or args.mode == 'single-rot':
        ...
        while True:
            ...
            if new_pos:
                if args.mode == 'rnd':
                    x, y = [random.uniform(-1.5, 1.5) for _ in range(2)]
                elif args.mode == 'basic' or args.mode == 'single-rot':
                    x, y = [0, 0]   # [0, 0]  # [1, -1]

                bpy.context.object.location[0] = x
                bpy.context.object.location[1] = y

            # Check to make sure the new object is not intersecting others
            intersect_good = True
            for obj2 in prev_objs:
                # intersect = bmesh_check_intersect_objects(obj, obj2)
                # if not intersect:
                #     intersect = bmesh_check_intersect_objects(obj2, obj)
                intersect = bb_intersect(obj, obj2, mindist=args.min_dist)
                # print("There are%s intersections." % ("" if intersect else " NO"))
                if intersect:
                    intersect_good = False
                    break
...
def bmesh_check_intersect_objects(obj, obj2):
    ...
    for ed in bm.edges:
        ...
        success, co, no, index = ray_cast(co_1, (co_2 - co_1).normalized(), ed.calc_length())
        if success:
            intersect = True
            break
...
def bb_intersect(obj1, obj2, mindist):
    c1 = obj1.location[:2]
    c2 = obj2.location[:2]

    theta1 = obj1.rotation_euler[2]
    theta2 = obj2.rotation_euler[2]

    rot1 = np.array([[np.cos(theta1), -np.sin(theta1)],
                     [np.sin(theta1), np.cos(theta1)]])
    rot2 = np.array([[np.cos(theta2), -np.sin(theta2)],
                     [np.sin(theta2), np.cos(theta2)]])

    dimx1 = obj1.dimensions.x / 2. + mindist
    dimz1 = obj1.dimensions.z / 2. + mindist
    dimx2 = obj2.dimensions.x / 2. + mindist
    dimz2 = obj2.dimensions.z / 2. + mindist

    verts1 = np.reshape(c1, (2, 1)) + np.matmul(rot1, np.array([[dimx1, -dimx1, -dimx1, dimx1],
                                                                [dimz1, dimz1, -dimz1, -dimz1]]))
    verts2 = np.reshape(c2, (2, 1)) + np.matmul(rot2, np.array([[dimx2, -dimx2, -dimx2, dimx2],
                                                                [dimz2, dimz2, -dimz2, -dimz2]]))

    verts1 = [(x, y) for x, y in verts1.transpose()]
    verts2 = [(x, y) for x, y in verts2.transpose()]

    return separating_axis_theorem(verts1, verts2)
...
def render_shadeless(blender_objects, path='flat.png', info_objects=None):
    ...
    old_materials = []
    for i, obj in enumerate(blender_objects):
        old_materials.append([mat for mat in obj.data.materials])
    ...
    for mat, obj in zip(old_materials, blender_objects):
        for j, mat_ in enumerate(mat):
            obj.data.materials[j] = mat_
...
def get_3x4_RT_matrix_from_blender(cam):
    ...
    print('rotation, degrees', math.degrees(rotation.x), math.degrees(rotation.y), math.degrees(rotation.z))
